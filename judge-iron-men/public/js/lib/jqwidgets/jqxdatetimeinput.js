...
(function(a) {
    ...
    a.extend(a.jqx._jqxDateTimeInput.prototype, {
                ...
                localization: {
                    ...
                    calendar: {
                      ...
                      months: {
                       ...
                       namesAbbr:["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Sett","Out","Nov","Dez",""]
                        }, 
                        AM: ["AM", "am", "AM"],
                        PM: ["PM", "pm", "PM"],
                        eras: [{
        ...
        _arrange: function() {
            ...
            this.timeButton[0].style.width = h + 1 + "px";
            if (this.showCalendarButton) {
                this.timeButton[0].style.left = 1 + this.calendarButton.width() + c + "px"
            } else {
                this.timeButton[0].style.left = 1 + c + "px"
            }
            ...
            if (this.rtl) {
                ...
                this.timeButton[0].style.width = h + "px";
                this.calendarButton.css("left", "0px");
                if (this.showCalendarButton) {
                    this.timeButton.css("left", h + "px")
                } else {
        ...
        _addHandlers: function() {
            ...
            if (this.calendarContainer != null) {
                this.addHandler(this.calendarContainer, "keydown", function(f) {
                    ...
                    if (f.keyCode == 115) {
                        if (d.isOpened()) {
                            d.hideCalendar("keyboard");
                            d.dateTimeInput.focus();
                            return false
                        } else {
                            if (!d.isOpened()) {
                                d.showCalendar("keyboard");
                                d.dateTimeInput.focus();
                                return false
                            }
                        }
        ...
        initTimePopup: function() {
            this.timePopupElement = a("<div style='border-style: solid; border-width: 1px;'></div>").appendTo(this.timeContainer);
        ...
        showCalendar: function() {
            ...
            var e, r = parseInt(Math.round(this.host.coord(true).left));
            e = r + "px";
            var w = a.jqx.mobile.isSafariMobileBrowser() || a.jqx.mobile.isWindowsPhone();
            if ((w != null && w)) {
            ...
            if (p.selectionMode == "default") {
                this._validateValue();
                this._updateText();
                var t = this.value != null ? this.value.dateTime : new Date();
                q.setDate(t)
            }
            ...
            if (this.dropDownHorizontalAlignment == "right" || this.rtl) {
                ...
                var u = Math.abs(k - this.host.outerWidth() + 2);
                if (!this.rtl) {
                    u -= 2
                }
                if (k > this.host.width()) {
                    var f = 23;
                    this.container.css("left", f + parseInt(Math.round(r)) - u + "px")
                } else {
                    this.container.css("left", 25 + parseInt(Math.round(r)) + u + "px")
                }
            }
            ...
            if (this.animationType != "none") {
                ...
                if (this.animationType == "fade") {
                    ...
                    if (this.timePopup) {
                        if (b) {
                            this.timeContainer.css("margin-top", o)
                        } else {
                            this.timeContainer.css("margin-top", -o)
                        }
                        this.timeContainer.animate({
            ...
            if (this.timePopup) {
                this.timeButtonIcon.addClass(this.toThemeProperty("jqx-icon-calendar-pressed"));
        ...
        hideCalendar: function(k) {
            ...
            this.timeButtonIcon.removeClass(this.toThemeProperty("jqx-icon-calendar-pressed"));
        ...
        updateValue: function() {
            ...
            for (var A = 0; A < this.items.length; A++) {
                switch (this.items[A].type) {
                    case "FORMAT_AMPM":
                        h = this.editors[A].value;
                        e = this.editors[A];
                        break;
                    case "Character":
                        break;
                    case "Day":
                        if (this.items[A].format.length < 3) {
                            B = this.editors[A].value;
                            t[y++] = this.editors[A];
                            if (B == 0) {
                                B = 1
                            }
                        ...
                        break;
                    case "FORMAT_hh":
                        var E = this.editors[A];
                        l = E.value;
                        c = true;
                        break;
                    case "FORMAT_HH":
                        l = this.editors[A].value;
                        c = true;
                        break;
                    case "Millisecond":
                        d = this.editors[A].value;
                        x = true;
                        break;
                    case "Minute":
                        u = this.editors[A].value;
                        b = true;
                        break;
                    case "Month":
                        H = this.editors[A].value;
                    ...
                    case "ReadOnly":
                        break;
                    case "Second":
                        I = this.editors[A].value;
                        q = true;
                        break;
                    case "Year":
                        m = true;
                        s = this.editors[A].value;
                        var G = this.editors[A].getDateTimeItem().format;
                        if (G.length < 3) {
                            var w = "1900";
                            if (w.Length == 4) {
                                var v = "" + w[0] + w[1];
            ...
            var F = this.value != null ? new Date(this.value.dateTime) : null;
            if (s > 0 && H > 0 && B > 0 && u >= 0 && l >= 0 && I >= 0 && d >= 0) {
                var J = this.value;
                if (J != null) {
                    if (!m) {
                        s = J.year
                ...
                try {
                    ...
                    if (J._daysInMonth(s, H) <= B) {
                        B = J._daysInMonth(s, H);
                    ...
                    this.value._setDay(B);
                    this.value._setMonth(H);
                    if (c) {
                ...
                if (F != null) {
                    var z = this.value.dateTime.getFullYear() == F.getFullYear() && this.value.dateTime.getDate() == F.getDate() && this.value.dateTime.getMonth() == F.getMonth() && this.value.dateTime.getHours() == F.getHours() && this.value.dateTime.getMinutes() == F.getMinutes() && this.value.dateTime.getSeconds() == F.getSeconds();
                    if (!z) {
                        if (this.changing) {
                            var n = this.changing(F, this.value.dateTime);
                            if (n) {
        ...
        _handleKeyDown: function(h, o) {
            ...
            if (h.keyCode == 115) {
                if (g.isOpened()) {
                    g.hideCalendar("keyboard");
                    return false
                } else {
                    if (!g.isOpened()) {
                        g.showCalendar("keyboard");
                        return false
                    }
                }
            }
